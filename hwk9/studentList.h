...
public:
	...
	student retrieveStudent(unsigned int idnumber)
	{
		...
		while (current->data.id != idnumber && current->next != NULL)
		{
	...
	// ADDITIONAL CODE FOR HWK9 //
	void removeStudent(unsigned int idnumber)
	{
		if (head == NULL)
			return;
